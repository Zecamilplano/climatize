...
const link = [{ label: "produtos", href: "#produtos" }]

export default function HomeComponent() {
  const pathname = usePathname()
  const { scrollToSection } = useHeaderScroll(pathname, link)

  return (
    <section id="inicio" className="bg-main flex gap-6 md:gap-7 flex-wrap justify-center items-center md:h-[100dvh] py-5">

      <article onClick={() => scrollToSection(link[0].href)} className="bg-white self-center rounded-md flex justify-center items-center text-center flex-col w-[360px] h-[180px] gap-2 desktop-2560:w-[618px] desktop-2560:h-[290px] desktop-2560:gap-6  transition-all duration-300 hover:shadow-lg active:scale-95 active:shadow-sm ">
