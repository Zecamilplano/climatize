...
import { DataToSendType, TypeCardInformation } from "@/app/types/types"
import { listCard } from "@/app/data/cards"
import "./style.css"
import { UseDragEvents } from "@/app/hooks/hook-drag-events"
import { redirect } from "next/navigation"
import { useCardInfo } from "@/app/contexts/card-info-context"
...
export default function NewProduct() {
  ...
  // const { newInfo, cardCard } = HooksCardInformation()
  const { cardData } = useCardInfo()
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const dadosParaEnvio: DataToSendType = {
      ...
      features: cardData["Características Principais"],
      benefits: cardData["Benefícios"],
      specifications: cardData["Especificações"],
    };
  ...
  return (
    <div className="min-h-[calc(100vh-50px)] pb-12 font-inter">
      ...
      <form onSubmit={handleSubmit} className="mx-3 grid md:gap-8 grid-cols-1 grid-form">
        {/* Imagem do Produto */}
            ...
            {images.length === 0 ? (
              ...
              images.map(image => (
                <div className={`relative w-full h-full group ${isDragging ? "opacity-50" : "opacity-100"}`} key={image.name}>
                  <Image src={image.preview} fill style={{ objectFit: "cover" }} alt="image" /> <X color="red" className="absolute top-0 right-0 bg-gray-600 hidden group-hover:block cursor-pointer" onClick={(e) => {
        ...
        <section style={{ gridArea: "benefits" }}>
          ...
          <div className="flex gap-6 justify-center items-end md:h-1/3 md:mt-3">
            ...
            >Cancelar</button>
            <button type="submit" className="h-10 bg-cyan-400 text-white flex justify-center items-center px-6 rounded-md gap-2"><Plus />Salvar produto</button>
          </div>
